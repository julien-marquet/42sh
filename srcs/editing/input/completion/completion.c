...
int			complete_arg(t_input_data *input, char *word, t_sh_state *state)
{
	...
	len = ft_strlen(word);
	if (len == 0)
		return (find_in_dir(get_files(".", "", CHK_NONE, state->internal_storage), input, ""));
	else if (word[len - 1] == '/')
	{
		if ((tmp = get_path(input, 0)) == NULL)
			return (1);
		if (find_in_dir(get_files(tmp, "", CHK_NONE, state->internal_storage), input, "") == 1)
			return (1);
		free(tmp);
	}
	else
	{
		...
		while (i > 0)
		{
			if (*word == '/' || (*word == '\\' && *(word - 1) != '\\'))
			{
				if ((tmp = get_path(input, 0)) == NULL)
					return (1);
				if (find_in_dir(get_files(tmp, word + 1, CHK_NONE, state->internal_storage), input, word + 1) == 1)
					return (1);
				free(tmp);
				return (0);
			}
		...
		return (find_in_dir(get_files(".", word, *word == '$' ? CHK_VARS : CHK_NONE, state->internal_storage), input, word));
	}
	return (0);
}
...
int			complete_bin(char *word, t_sh_state *sh_state, t_input_data *input)
{
	...
	while (*pointer != NULL)
	{
		lstmerge(&files, get_files(*pointer, word, CHK_NONE, sh_state->internal_storage));
		pointer += 1;
	}
	lstmerge(&files, get_files(NULL, word, *word == '$' ? CHK_BUILTINS & CHK_VARS : CHK_BUILTINS, sh_state->internal_storage));

...
/*   Updated: 2019/05/03 23:28:48 by mmoya       ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
int		handle_user_reset(t_input_data *input_data, t_sh_state *sh_state)
{
	if (input_data->sig_call == 1)
	{
		if (get_eof() == 0)
		{
			...
			{
				if (add_to_history_list(&(sh_state->history), input_data->active_buf->buf, input_data->active_buf->len + 1) == NULL)
					return (1);
			}
			else if (input_data->stored_buf->len > 0 && input_data->stored_buf->buf[0] != '\n')
			{
				if (add_to_history_list(&(sh_state->history), input_data->stored_buf->buf, input_data->stored_buf->len + 1) == NULL)
					return (1);
			}
		}

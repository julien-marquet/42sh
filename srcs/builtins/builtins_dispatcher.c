...
static t_builtin_func	get_builtins_func(const char *name)
{
	static char	*assoc_name[BUILTINS_NB + 1] = {
		"set", "env", "setenv", "unsetenv", "unset", "exit", "echo", "export", "test", "[", "alias", "unalias", NULL
	};
	static int	(*assoc_func[BUILTINS_NB + 1])(t_sh_state *, int, const char **, t_builtin_context *) = {
		...
		builtin_unset, builtin_exit, builtin_echo, builtin_export, builtin_test, builtin_test, builtin_alias, builtin_unalias, NULL
	};
	size_t		i;
